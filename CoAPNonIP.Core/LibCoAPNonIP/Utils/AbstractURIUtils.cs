...
namespace LibCoAPNonIP.Utils {
    public abstract class AbstractURIUtils {
        ...
        public static bool IsValidFullUri(string uri) {
            ...
            if (uri.IndexOf("://") == -1)
                return false;
            if (AbstractURIUtils.GetUriScheme(uri) == null)
                return false;
            if (uri.IndexOf("://") + 1 == uri.Length)
                return false; //string is of the format xx:// only, no host
            return true;
        }
        ...
        public static string GetQueryParameterValue(string uri, string qpName) {
            if (uri == null || uri.Trim().Length == 0)
                return null;
            if (qpName == null || qpName.Trim().Length == 0)
                return null;
