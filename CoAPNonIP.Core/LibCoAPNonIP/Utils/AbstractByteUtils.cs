...
namespace LibCoAPNonIP.Utils {
    public abstract class AbstractByteUtils {
        ...
        public static void UInt16_To_BE(UInt16 input, byte[] bs) {
            if (bs == null)
                throw new ArgumentNullException("Byte stream/array is null");
            if (bs.Length < 2)
                throw new ArgumentException("Byte stream/array size too small");
        ...
        public static void UInt64_To_LE(UInt64 input, byte[] bs) {
            if (bs == null)
                throw new ArgumentNullException("Byte stream to output data is null");
            if (bs.Length < 8)
                throw new ArgumentException("Byte stream too short");
            bs[0] = (byte)input;
            bs[1] = (byte)(input >> 8);
            bs[2] = (byte)(input >> 16);
            bs[3] = (byte)(input >> 24);
