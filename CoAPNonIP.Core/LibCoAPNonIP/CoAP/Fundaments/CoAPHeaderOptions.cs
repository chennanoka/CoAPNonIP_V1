...
namespace LibCoAPNonIP.CoAPMsg {
    public class CoAPHeaderOptions : ArrayList , InterfaceParsable {
        #region Constructors
        ...
        public int Parse(byte[] coapMsgStream, int startIndex, UInt16 extraInfo) {
            if (coapMsgStream == null || coapMsgStream.Length <= AbstractCoAPMsg.HEADER_LENGTH ||
                startIndex < AbstractCoAPMsg.HEADER_LENGTH || startIndex >= coapMsgStream.Length)
        ...
        public bool IsValid(UInt16 value) {
            foreach (CoAPHeaderOption coapHeaderOption in this) {
                if (!coapHeaderOption.IsValid(coapHeaderOption.Number))
                    return false;
            }
            return true;
        }
        ...
        public ArrayList GetOptions(UInt16 optionNumber) {
            ...
            foreach (CoAPHeaderOption headerOption in this)
                if (headerOption.Number == optionNumber)
                    matchingOptions.Add(new CoAPHeaderOption(headerOption.Number, headerOption.Value));
