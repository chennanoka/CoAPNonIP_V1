...
namespace LibCoAPNonIP.CoAPMsg {
    public class CoAPMsgCode : InterfaceParsable {
        #region Constants
        ...
        public byte Value {
            ...
            set {
                if (!this.IsValid(value))
                    throw new CoAPFormatException("Invalid message code");
                this._mCode = value;
            }
        }
        ...
        public CoAPMsgCode(byte mCode) {
            if (!this.IsValid(mCode))
                throw new ArgumentException("Invalid message code");
            this._mCode = mCode;
        }
        ...
        public static bool DoesMessageCodeRepresentARequest(byte mCode) {
            return (mCode == CoAPMsgCode.DELETE ||
            mCode == CoAPMsgCode.GET ||
            mCode == CoAPMsgCode.POST ||
            mCode == CoAPMsgCode.PUT ||
            mCode == CoAPMsgCode.EMPTY /*special case*/);
        }
        ...
        public bool IsValid(UInt16 value) {
            ...
            if (classCode != 0 && classCode != 2 && classCode != 4 && classCode != 5)
                return false;
            if (classCode == 0 && detailCode > 4)
                return false;
            if (classCode == 2 && (detailCode == 0 || (detailCode > 5 && detailCode != 31)))
                return false;
            if (classCode == 4 && (detailCode == 7 || detailCode == 9 || detailCode == 10 ||
                detailCode == 11 || detailCode == 14 || detailCode > 15))
                return false;
            if (classCode == 5 && detailCode > 5)
                return false;
            return true;
        }
