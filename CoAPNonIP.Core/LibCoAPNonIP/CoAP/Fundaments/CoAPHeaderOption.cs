...
namespace LibCoAPNonIP.CoAPMsg {
    public class CoAPHeaderOption : InterfaceParsable {
        #region Constants
        ...
        public int Parse(byte[] coapStream, int startIndex, UInt16 previousOptionNumber) {
            if (coapStream == null || coapStream.Length < startIndex || startIndex < AbstractCoAPMsg.HEADER_LENGTH)
        ...
        public bool IsValid(UInt16 value) {
            if (value != CoAPHeaderOption.ACCEPT && value != CoAPHeaderOption.BLOCK1 &&
                ...
                value != CoAPHeaderOption.URI_PORT && value != CoAPHeaderOption.URI_QUERY)
                return false;
        ...
        public int GetOptionValueMaxLengthInBytes(int optionNumber) {
            switch (optionNumber) {
                ...
                case CoAPHeaderOption.URI_QUERY:
                    return 255; 
                default:
                    return 1024;//TODO::No guidance for options that are not recognized...
            }            
        }
